	...
	public class ColorHelper
	{
        ...
        public static Color ColorFromString(string color)
        {
            try
            {
                ...
                byte byteR = (byte)Convert.ToByte(splittedColor[0].ToString().TrimStart('R').TrimStart(':'));
                byte byteG = (byte)Convert.ToByte(splittedColor[1].ToString().TrimStart('G').TrimStart(':'));
                byte byteB = (byte)Convert.ToByte(splittedColor[2].ToString().TrimStart('B').TrimStart(':'));
                byte byteA = (byte)Convert.ToByte(splittedColor[3].ToString().TrimStart('A').TrimStart(':'));

                return new Color(byteR, byteG, byteB, byteA);
            }
            catch
            {
                //Red = Error
        ...
        public static string ColorToString(Color color)
        {
            try
            {
                return color.ToString().TrimStart('{').TrimEnd('}');
            }
            catch
            {
                return string.Empty;
